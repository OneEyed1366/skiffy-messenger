...
| Task ID | Name                          |
| ------- | ----------------------------- |
| T8.11   | usePermissions                |
| T10d.\* | All permission-gated features |

## Description

Migrate permission constants from Mattermost. These constants define all permission strings used for role-based access control (RBAC) throughout the application.

## Source Files

- `vendor/desktop/webapp/channels/src/packages/mattermost-redux/src/constants/permissions.ts`
- `vendor/desktop/webapp/channels/src/packages/mattermost-redux/src/constants/roles.ts`
- `vendor/desktop/webapp/channels/src/utils/constants.tsx` (PermissionsScope, DefaultRolePermissions, ModeratedPermissions)

## Target File

`apps/v2/src/constants/permissions.ts`
...
export const PERMISSIONS = {
  ...
  //#region Post & Reaction Permissions
  ...
  //#region Role & System Permissions
  ...
  //#region Integration Permissions
  ...
  //#region Access Token Permissions
  ...
  //#region Custom Group Permissions
  ...
  //#region Shared Channels Permissions
 ...
 * Migrated from: vendor/.../utils/constants.tsx
 */
 ...
 * Migrated from: vendor/.../constants/roles.ts
 */
...
- [ ] All permission constants defined with `as const`
- [ ] Types prefixed with `I` (IPermission, IPermissionScope)
- [ ] Uses `#region` comments for organization
- [ ] Exported from `constants/index.ts`
- [ ] No `any` types
- [ ] TypeScript strict mode passes
- [ ] Permission scope mapping included
- [ ] Channel moderated permissions included
- [ ] Role constants (team, channel, system, playbook) included
- [ ] Role groups (members, guests, admins) included
...
- System console permissions (SYSCONSOLE\_\*) are intentionally excluded as they are admin-specific and unlikely to be needed in the mobile/desktop app
- The PERMISSIONS object uses `as const` for type-safe string literals
- PERMISSION_SCOPE maps each permission to its scope (system, team, channel, playbook, run)
- MODERATED_PERMISSIONS lists permissions that can be restricted per-channel
