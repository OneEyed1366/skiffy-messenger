...
- [ ] Modify MatrixClient login method signature (AC: 1)
  - [ ] Update `rust/src/core/matrix_client/client.rs` to accept `&dyn SecureStorage` parameter
  - [ ] Update FFI API in `rust/src/api/auth.rs` to pass SecureStorage instance
  - [ ] Modify login implementation to store tokens immediately after successful authentication
  - [ ] Store `access_token`, `refresh_token`, `user_id`, and `device_id` with standardized keys

- [ ] Implement session restoration functionality (AC: 2)
  - [ ] Create `restore_session` method in MatrixClient
  - [ ] Load stored credentials from SecureStorage on startup
  - [ ] Initialize matrix-rust-sdk with restored session data
  - [ ] Add error handling for missing or invalid stored credentials
  - [ ] Expose restore_session via FFI layer

- [ ] Update logout implementation (AC: 3)
  - [ ] Modify `logout` method to clear all session data from SecureStorage
  - [ ] Clear `access_token`, `refresh_token`, `user_id`, and `device_id`
  - [ ] Ensure server-side logout is performed before clearing local data
  - [ ] Handle partial failure scenarios gracefully

- [ ] Integration testing and security verification (AC: 4)
  - [ ] Write unit tests for modified login method with SecureStorage
  - [ ] Write unit tests for restore_session functionality
  - [ ] Write unit tests for logout with SecureStorage cleanup
  - [ ] Verify no plaintext credentials are stored anywhere
  - [ ] Test fallback scenarios (Linux in-memory storage)

- [ ] Update Flutter integration layer
  - [ ] Modify AuthBloc to use updated login/logout methods
  - [ ] Implement app startup session restoration flow
  - [ ] Handle session restoration success/failure in UI
